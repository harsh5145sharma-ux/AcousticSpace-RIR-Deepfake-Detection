...
function mainThreadFrequencies(signal, fftSamples, windowFunc, alpha) {
    const fft = new FFT(fftSamples, SAMPLE_RATE, windowFunc, alpha);
...
function workerOptions(signal, fftSamples, windowFunc, alpha) {
    return {
        startTime: 0,
...
describe('spectrogram worker', () => {
    it('uses the same default window alpha as the main thread when alpha is not set', () => {
        ...
        workerResult[0].forEach((frame, i) => {
