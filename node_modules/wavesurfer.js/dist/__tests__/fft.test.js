import FFT, { applyFilterBank, applySparseFilterBank, createFilterBankForScale, createSparseFilterBankForScale, } from '../fft.js';
const SCALES = ['mel', 'logarithmic', 'bark', 'erb'];
/** Deterministic pseudo-random spectrum (LCG) so parity failures are reproducible */
function makeSpectrum(length, seed = 1) {
...
function makeSine(length, frequency, sampleRate) {
...
describe('FFT zero-padding (windowLength)', () => {
    ...
    it('defaults windowLength to the buffer size with unchanged output', () => {
        ...
        const spectrum = new FFT(512, SAMPLE_RATE, 'hann', undefined).calculateSpectrum(signal);
        const spectrumExplicit = new FFT(512, SAMPLE_RATE, 'hann', undefined, 512).calculateSpectrum(signal);
    ...
    it('keeps bin magnitudes calibrated when the window is zero-padded', () => {
        ...
        const unpadded = new FFT(64, SAMPLE_RATE, 'hann', undefined).calculateSpectrum(signal);
        const frame = new Float32Array(512);
        frame.set(signal);
        const padded = new FFT(512, SAMPLE_RATE, 'hann', undefined, 64).calculateSpectrum(frame);
    ...
    it('produces finite, correctly sized output for non-linear scales with a padded FFT', () => {
        ...
        const spectrum = new FFT(fftSize, SAMPLE_RATE, 'hann', undefined, fftSamples).calculateSpectrum(frame);
    ...
    it('masks stale samples beyond the window with the zeroed window tail', () => {
        ...
        const fft = new FFT(512, SAMPLE_RATE, 'hann', undefined, 64);
    ...
    it('throws when windowLength exceeds the buffer size', () => {
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, 1024)).toThrow();
    });
    it('throws when windowLength is shorter than two samples', () => {
        ...
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, 1)).toThrow();
    });
    it('rejects explicit invalid windowLength values instead of coercing them', () => {
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, 0)).toThrow();
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, NaN)).toThrow();
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, 80.5)).toThrow();
        expect(() => new FFT(512, SAMPLE_RATE, 'hann', undefined, -64)).toThrow();
    });
});
