...
function createSparseFilterBank(numFilters, fftSamples, sampleRate, hzToScaleFunc, scaleToHzFunc) {
    const filterMin = hzToScaleFunc(0);
    const filterMax = hzToScaleFunc(sampleRate / 2);
...
export function applySparseFilterBank(fftPoints, filterBank) {
    const numFilters = filterBank.length;
...
export function createSparseFilterBankForScale(scale, numFilters, fftSamples, sampleRate) {
    switch (scale) {
        case 'mel':
 ...
 */
function FFT(bufferSize, sampleRate, windowFunc, alpha, windowLength) {
    // Absent values default to the full buffer; explicit values are validated, not coerced
    windowLength = windowLength !== null && windowLength !== void 0 ? windowLength : bufferSize;
    ...
    switch (windowFunc) {
        ...
        case 'gauss':
            alpha = alpha || 0.25;
            for (let i = 0; i < windowLength; i++) {
                this.windowValues[i] = Math.pow(Math.E, -0.5 * Math.pow((i - (windowLength - 1) / 2) / ((alpha * (windowLength - 1)) / 2), 2));
            }
            break;
        case 'hamming':
            for (let i = 0; i < windowLength; i++) {
