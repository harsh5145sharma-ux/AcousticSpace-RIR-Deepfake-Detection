...
function calculateFrequencies(audioChannels, options) {
    const { startTime, endTime, sampleRate, fftSamples, fftSize, windowFunc, alpha, noverlap, scale, gainDB, rangeDB, splitChannels, } = options;
    const startSample = Math.floor(startTime * sampleRate);
    const endSample = Math.floor(endTime * sampleRate);
    const channels = splitChannels ? audioChannels.length : 1;
    const fftLength = fftSize !== null && fftSize !== void 0 ? fftSize : fftSamples;
    ...
    if (!fft || fft.bufferSize !== fftLength || fft.windowLength !== fftSamples) {
        fft = new FFT(fftLength, sampleRate, windowFunc, alpha, fftSamples);
    }
